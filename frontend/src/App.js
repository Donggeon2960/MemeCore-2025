...
const WEEKLY_EVENTS = [
  { 
    ...
    tokenIcon: "🐸", 
    tokenPrice: 65, 
    description: "가장 슬픈 개구리 밈을 만들어주세요! 웃픈 사연 대환영!", 
    endDate: "2025-12-31", // [수정] 날짜를 미래로 설정 -> 진행중
    // status 필드는 이제 사용하지 않으므로 없어도 됩니다.
  },
  { 
    id: 102, 
    theme: "도지 투 더 문", 
    ...
    tokenIcon: "🐕", 
    tokenPrice: 98, 
    description: "도지코인 화성 갈끄니까~ 관련 밈이나 영상을 올려주세요.", 
    endDate: "2024-12-01", // [수정] 날짜를 과거로 설정 -> 종료됨
  }
];
...
function QuestCard({ quest, onClick }) {
  // 완료된 퀘스트면 스타일 변경 (투명도 낮추고, 테두리 초록색)
  const cardStyle = quest.isCompleted 
    ? { 
        ...
        opacity: 0.6,           // 흐리게 처리
        border: '1px solid #4ade80' // 성공의 초록 테두리
      } 
    : { 
        minHeight: '200px', 
        display: 'flex', 
  ...
  return (
    <div 
      ...
      style={cardStyle} 
      // 완료된 퀘스트는 클릭해도 아무 일도 안 일어나게 함
      onClick={!quest.isCompleted ? onClick : undefined} 
    >
      <div>
        ...
        <h4 className="font-bold text-lg mb-2" style={{textDecoration: quest.isCompleted ? 'line-through' : 'none'}}>
          {quest.title}
        </h4>
      ...
      <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-700">
        <span className={`font-bold ${quest.isCompleted ? 'text-gray-500' : 'text-green-400'}`}>
          {quest.rewardPoints} P
        </span>
        
        <button 
          className="btn-action text-xs" 
          // 완료되었으면 버튼 비활성화 및 색상 변경
          style={{ 
            background: quest.isCompleted ? '#334155' : 'var(--primary)',
            cursor: quest.isCompleted ? 'not-allowed' : 'pointer'
          }}
          onClick={(e) => { 
            e.stopPropagation(); 
            if(!quest.isCompleted) onClick(); 
          }}
    ...
    return (
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
        ...
        <div className="login-box">
          ...
          {preview ? (
            <img src={preview} alt="" style={{width:'100%', borderRadius:'8px', marginBottom:'1rem', border:'1px solid #555'}} />
          ) : (
            <div className="text-center p-8 border border-dashed border-gray-500 rounded-lg text-sub mb-4">
...
function SnsQuestSection({ user, onUpdateUser }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('http://localhost:3000/api/submit-social', {
        method: 'POST',
        body: formData
      });
      const data = await res.json();
  ...
  return (
    <div className="login-box" style={{ maxWidth: '600px', margin: '0 auto' }}>
      {loading && <LoadingOverlay message="AI가 스크린샷 텍스트를 분석 중입니다..." />}
      ...
      <div className="mb-6 p-4 bg-gray-800 rounded-lg border border-gray-600">
        ...
        <ol className="text-sm space-y-2 pl-4 text-gray-300">
          <li>SNS(유튜브, 인스타 등)에 밈을 업로드하세요.</li>
        ...
        <div className="flex gap-2 mb-4">
          {['YouTube', 'Instagram', 'TikTok', 'X'].map(p => (
            <button 
              ...
              style={{ flex: 1, padding: '0.5rem', borderRadius: '0.5rem', border: platform === p ? '2px solid #f59e0b' : '1px solid #334155', background: 'transparent', color: 'white', cursor: 'pointer' }}
            >
              {p}
            </button>
          ))}
      ...
      <button onClick={handleSubmit} disabled={loading} className="login-btn" style={{ background: loading ? '#334155' : '#f59e0b', color: 'black' }}>
        스크린샷 제출하고 300P 받기
      </button>
    </div>
  );
    ...
    <div style={{ maxWidth: '1024px', margin: '0 auto' }}>
      <div style={{ display: 'flex', gap: '30px', marginBottom: '1.5rem' }}>
        <button 
          onClick={() => setTab('ai')} 
          className="login-btn"
          style={{ flex: 1, margin: 0, background: tab === 'ai' ? '#9333ea' : '#1e293b', color: 'white' }}
        >
          🤖 AI 밈 챌린지
        </button>
        <button 
          onClick={() => setTab('sns')} 
          className="login-btn"
          style={{ flex: 1, margin: 0, background: tab === 'sns' ? '#f59e0b' : '#1e293b', color: tab === 'sns' ? 'black' : 'white' }}
        >
          📢 SNS 바이럴
        </button>
      </div>
              ...
              <div key={evt.id} className="event-card" onClick={() => setSelectedEvent(evt)} style={{cursor: 'pointer', opacity: isExpired ? 0.7 : 1}}>
                <div className="event-header">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="font-bold">{evt.theme}</h4>
                      <span className="badge badge-purple">{evt.tokenSymbol} 보상</span>
                    </div>
                  </div>
                  {/* [핵심 수정] 상태에 따라 배지 색상과 텍스트 변경 */}
                  <span className="badge" style={{ 
                    backgroundColor: isExpired ? 'rgba(239, 68, 68, 0.1)' : 'rgba(74, 222, 128, 0.1)', 
                    color: isExpired ? '#ef4444' : '#4ade80',
                    border: `1px solid ${isExpired ? 'rgba(239, 68, 68, 0.2)' : 'rgba(74, 222, 128, 0.2)'}`
                  }}>
                    {isExpired ? "종료됨" : "진행중"}
                  </span>
                </div>
                ...
                <button className="btn-action" style={{width:'100%', background: isExpired ? '#334155' : 'var(--primary)'}}>
                  {isExpired ? "결과 보기" : "참여하기 →"}
                </button>
              </div>
            );
          })}
...
// App.js 내부의 EventDetailView 컴포넌트 수정
...
function EventDetailView({ event, user, onBack }) {
  ...
  // [NEW] 날짜 확인: 오늘 날짜가 마감일(endDate)을 지났는지 체크
  ...
  const handleVote = async (id) => {
    // 만료된 이벤트라도 투표는 가능하게 할지, 불가능하게 할지 선택 (여기선 가능하게 둠)
    await fetch('http://localhost:3000/api/like-image', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ login_id: user.username, submission_id: id })
    });
    fetchEntries();
  };
  ...
  return (
    <div style={{ maxWidth: '1024px', margin: '0 auto' }}>
      ...
      <button onClick={onBack} className="text-sub mb-4">&larr; 목록으로</button>
      ...
      <div className="p-6 bg-gray-800 rounded-xl mb-8 text-center" style={{border: isExpired ? '2px solid #ef4444' : 'none'}}>
        ...
        {/* 마감일 표시 */}
        <div className="mb-6 text-sm font-bold" style={{color: isExpired ? '#ef4444' : '#4ade80'}}>
          {isExpired ? `🚫 이 이벤트는 종료되었습니다 (${event.endDate} 마감)` : `📅 마감일: ${event.endDate} 까지`}
        </div>
        ...
        {!isExpired ? (
          <div className="login-box" style={{maxWidth: '500px', margin: '0 auto'}}>
            ...
            <div className="flex gap-2">
              <label className="btn-action flex-1 text-center cursor-pointer" style={{background:'#334155'}}>
                <input type="file" hidden onChange={e => {setFile(e.target.files[0]); setPreview(URL.createObjectURL(e.target.files[0]))}} />
                <Upload style={{display:'inline', marginRight:'5px'}} /> 이미지 선택
              </label>
              <button onClick={handleUpload} className="btn-action flex-1">출품하기</button>
            </div>
          </div>
        ) : (
          <div className="p-4 bg-gray-900 rounded text-sub">
            아쉽지만 참여 기간이 지났습니다. 다른 참가자들의 작품을 감상하고 투표해주세요!
          </div>
        )}
      </div>
...
function GalleryPage({ user }) {
  ...
  useEffect(() => {
    if (selectedImg) {
      const updated = images.find(img => img.id === selectedImg.id);
      if (updated) setSelectedImg(updated);
    }
  }, [images]);
  ...
  const handleComment = async () => {
    ...
    await fetch('http://localhost:3000/api/comment', {
      method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ login_id: user.username, submission_id: selectedImg.id, content: comment })
    });
            ...
            <div style={{ padding: '1rem' }}>
              ...
              <div className="flex justify-between items-center text-xs text-sub">
                ...
                <button 
                  ...
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: img.isLiked ? '#f43f5e' : 'white' }} 
                >
        ...
        <div style={{position: 'fixed', top:0, left:0, right:0, bottom:0, background: 'rgba(0,0,0,0.8)', zIndex: 100, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '1rem'}} onClick={() => setSelectedImg(null)}>
          <div style={{background: '#1e293b', padding: '1.5rem', borderRadius: '1rem', maxWidth: '500px', width: '100%', maxHeight: '90vh', overflowY: 'auto'}} onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-4 p-4 bg-gray-800 rounded">
              ...
              {selectedImg.comments && selectedImg.comments.map(c => (
                <div key={c.id} className="text-xs mb-1">
                  <span className="text-accent">{c.user.nickname}:</span> {c.content}
                </div>
              ))}
            </div>
        ...
        {rankings.map((ranker, index) => (
          <div key={index} className="flex items-center justify-between" style={{ padding: '1rem', borderBottom: '1px solid #334155' }}>
            <div className="flex items-center gap-4">
              <span className="font-bold" style={{ fontSize: '1.2rem', color: index < 3 ? '#facc15' : 'white', width: '20px' }}>{index + 1}</span>
              <div><div className="font-bold">{ranker.nickname}</div><div className="text-xs text-sub">{ranker.userCode}</div></div>
            </div>
...
function ExchangePage({ user, onUpdateUser }) {
  ...
  const handleSwap = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch('http://localhost:3000/api/swap-to-token', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ login_id: user.username, amount: Number(amount) })
      });
      const data = await res.json();
      if (data.status === 'success') {
    ...
    <div style={{ maxWidth: '600px', margin: '0 auto' }}>
      ...
      <div className="login-box">
        ...
        <div className="input-group">
          ...
          <input type="text" className="login-input" value={user.username} readOnly disabled style={{ opacity: 0.7, cursor: 'not-allowed', background: '#334155' }} />
        </div>

        <div className="input-group">
          <label className="input-label">현재 연결된 지갑 주소</label>
          <input type="text" className="login-input" value={user.walletAddress || "등록된 지갑 없음"} readOnly disabled style={{ opacity: 0.7, cursor: 'not-allowed', background: '#334155', fontSize: '0.9rem' }} />
        </div>

        <hr style={{ borderColor: '#334155', margin: '1.5rem 0' }} />

        <div className="input-group">
          <label className="input-label">닉네임 변경 (월 1회 가능)</label>
          <input type="text" className="login-input" value={nickname} onChange={e => setNickname(e.target.value)} />
        </div>
        <div className="input-group">
          <label className="input-label">지갑 주소 변경 (새로운 주소 입력)</label>
          <input type="text" className="login-input" placeholder="변경할 경우에만 0x... 주소 입력" value={wallet} onChange={e => setWallet(e.target.value)} />
        </div>
        <div className="input-group">
          <label className="input-label">새 비밀번호</label>
          <input type="password" className="login-input" placeholder="변경할 경우만 입력" value={password} onChange={e => setPassword(e.target.value)} />
        </div>
...
function LoginScreen({ onLogin }) {
  ...
  const handleSubmit = () => {
    ...
    if (isSignup) {
      ...
      if (!walletAddress) return alert("지갑 주소를 입력해주세요.");
    }
    ...
    <div className="login-container">
      <div className="login-wrapper">
        <div className="login-logo-section">
          <div className="login-logo-box"><span>M</span></div>
          <h2 className="font-bold" style={{fontSize: '2rem', margin: '0'}}>MemeQuest</h2>
          <p className="text-sub">{isSignup ? "새로운 계정을 생성하세요." : "로그인하여 퀘스트를 시작하세요."}</p>
        </div>
        
        <div className="login-box">
          <div className="input-group">
            <label className="input-label">아이디</label>
            <input type="text" className="login-input" placeholder="ID 입력" value={formData.id} onChange={e => setFormData({...formData, id: e.target.value})} />
            <p className="text-xs text-gray-500 mt-1">로그인에 사용할 영문 ID를 입력하세요.</p>
          </div>
          <div className="input-group">
            <label className="input-label">비밀번호</label>
            <input type="password" className="login-input" placeholder="비밀번호 입력" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} />
            <p className="text-xs text-gray-500 mt-1">안전한 비밀번호를 입력하세요.</p>
          </div>
          {isSignup && (
            <>
              <div className="input-group">
                <label className="input-label">닉네임</label>
                <input type="text" className="login-input" placeholder="사용할 닉네임" value={formData.nickname} onChange={e => setFormData({...formData, nickname: e.target.value})} />
                <p className="text-xs text-gray-500 mt-1">서비스 내에서 보여질 이름입니다.</p>
              </div>
              <div className="input-group">
                <label className="input-label">지갑 주소</label>
                <input type="text" className="login-input" placeholder="0x..." value={formData.walletAddress} onChange={e => setFormData({...formData, walletAddress: e.target.value})} />
                <p className="text-xs text-gray-500 mt-1">보상을 받을 이더리움 계열 지갑 주소를 입력하세요.</p>
              </div>
            </>
          )}
          <button onClick={handleSubmit} className="login-btn" style={{marginTop: '1rem'}}>{isSignup ? "회원가입 완료" : "로그인"}</button>
          <div className="text-center mt-4 text-sm">
            <span className="text-sub">{isSignup ? "이미 계정이 있나요?" : "아직 계정이 없나요?"} </span>
            <button onClick={() => setIsSignup(!isSignup)} className="text-accent font-bold" style={{background:'none', border:'none', cursor:'pointer'}}>{isSignup ? "로그인하기" : "회원가입하기"}</button>
          </div>
        </div>
      </div>
    </div>
  ...
  const handleAuth = async (mode, id, password, nickname, walletAddress) => {
    try {
      ...
      const body = mode === 'register' 
        ? { login_id: id, password, nickname, wallet_address: walletAddress }
      ...
      if (!res.ok) {
        ...
        alert(errorData.message || "실패했습니다.");
        return;
      }
      ...
      if (data.id) {
        setUser({ 
          ...
          wallet: { mq: data.points, pepe: 0, doge: 0 } 
        });
      }
